...
\begin{lstlisting}[morekeywords={sn},numbers=left,
caption=Annotating ``state space'' in \cref{lst:los},label=lst:losa]
...
\cref{fig:state-space}. The \lstinline|\definame| in line 3 introduces a
\textbf{verbalization} ``state space'' for the symbol \lstinline|state-space?state space|
...
\begin{smodule}[title=State Space]{state-space}
[... some imports ...]
\symdecl*{state space}
\begin{sdefinition}
  [...] we call the \sn{graph} [...] the \definame{state space} of $\Pi$.
\end{sdefinition}
\end{smodule}
\end{lstlisting}
...
For the annotation \lstinline|\sn{state-space}|, the author has to be aware ``state
space'' is a technical term, that the module from \cref{fig:state-space} exists, know the
...
possibly removing directives that become redundant by the new one. At ca 150.000 words in
...
interactions that allow to fine-tune the wordwise annotation workflow. \Cref{fig:srify}
...
\begin{figure}[ht]
  ...
  \caption{\srf in Action: Annotating the term ``state space''.}\label{fig:srify}
\end{figure}
...
check-mark); otherwise \srf would offer the annotator the choice to import it -- if we are
...
\lstinline|S| skips and adds a comment to always skip this word, and -- if \srf first
suggests the term `state` \lstinline|n| extends it to the compound ``state space'' by
including the next word.

\srf is usually called on a set of files -- e.g. in a directory or math archive -- which
together form a \textbf{session}, which can be interrupted and resumed without having to
re-do all the \lstinline|s| skips. Alternatively, \srf can be called e.g. on the top-level
...
\ednote{continue} \srf is open source\ednote{MK@FS: do we have an explicit license yet in
...
NER-based approach, after the \srf annotation process might change the trade-offs
