...
\author[1]{Michael Kohlhase}
\author[2]{Jan Frederik Schaefer}
...
As a running example, we will use the simple declarational sentence
\begin{quote}
Let $n$ be a natural number.
\end{quote}
where we want to align the concept reference ``natural number'' with
the corresponding WikiData identifier.
...
For our example there are, in fact, three possible alignment targets:
\begin{compactitem}
...
which puts the burden of creating alignments on human annotators --
ideally the authors.

The most obvious workflow is rather inefficient:
...
A more efficient workflow, supported by dedicated tools,
is crucial for large-scale alignment efforts.

The sTeX ecosystem, which uses diverse semantic markup to drive
learning support systems,
faced a similar challenge, which was recently addressed by the snify system.
...
The snify tool is a simple command line application that
steps through the phrases that could be annotated in a document
and presents the user with a list of potential targets.
\Cref{fig:snify} shows the tool in action.
The user can select the correct entry from the list by typing its number,
...
Aside from choosing the correct annotation,
snify has many other commands to skip
phrases, undo annotations, modify the selection, etc.
...
\begin{figure}
    ...
    \caption{Screenshot of snify.}\label{fig:snify}
\end{figure}
...
As WikiData also has formula notations
for many concepts, including \LaTeX\ notations like \lstinline.\mathbb{N}.,
we can also use the same workflow to align formula fragments.
Such formula alignment is rather limited:
...
in collaboration with a human annotator was presented in\ednote{TODO: Luca's work}.
...
As a case study, we have added support for HTML documents,
...
To improve this, we have also created a browser-based interface
...
to help alignment authors -- be it text-to-database or database-to-database alignment.
