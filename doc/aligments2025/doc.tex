...
An alignment links a concept from one system to a concept in another system,
...
Authors and curators often try to generate alignments from existing datasets
...
we build on the snify architecture for selection-based,
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Proposed flowchart for tool support for human aligners.}\label{fig:workflow}
\end{figure}
...
To support the workflow in \Cref{fig:workflow},
...
The snify tool~\cite{KohSch:safc25} supports semantic annotation with
a workflow similar to the one shown in \Cref{fig:workflow}
for semantic annotation.
Both the harvester and the frontend act on \LaTeX\ documents
that use the sTeX package for semantic markup.
...
and a frontend for aligning HTML documents along with
...
The snify tool was implemented as a simple command line application.
It steps through the phrases in a document that match verbalizations
from the concept glossary
and presents the user with a list of annotation candidates.
...
where the created alignments are stored in HTML attributes.
