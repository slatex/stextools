% \documentclass[notes]{notesslides}
...
\sf
    ...
    \begin{minipage}[t]{8.5cm}
        \begin{itemize}
            \item \blue{Note:}
                % We only have utilities for terminal states.
                % Depth-limited minimax requires a heuristic.
                % We only have \sr{utility}{utilities} for \sr{goal state}{terminal states}.
                % \sr{depth limit}{Depth-limited} \sn{minimax} requires a \sn{heuristic}.
                \sr{depth limit}{Depth-limited} \sn{minimax} requires an \sr{evaluation function}{evaluation} for every \sn{cut-off state} $s$.
                If $s$ is \sr{goal state}{terminal}, we use its \sn{utility}. Otherwise, we use an estimate.
        \end{itemize}
    \end{minipage}
    \hskip 0.5cm
    % \hfill
