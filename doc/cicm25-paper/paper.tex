...
\Cref{fig:snify} shows a typical situation.
...
\begin{wrapfigure}r{5.5cm}
  \begin{tikzpicture}[scale=.7]
    ...
    \node[doc] (cc) at (0,0) {\shortstack{Content\\ Commons}};
    \node[doc] (pl) at (5,0) {\shortstack{Private/\\Local}};
    \node[database] (db) at (3,2.5) {\shortstack{\tiny Verbalization\\Cache}};
    \node (sn) at (0,4.5) {\texttt{snify}};
    \node (id) at (2.5,5) {IDE};
    \node (wo) at (5,4.5) {WOIDE};
    \draw[include]  (cc) -- (db);
    \draw[includeleft] (pl) -- (db);
    \draw[->] (db) -- (sn);
    \draw[->] (db) -- (id);
    \draw[->] (db) -- (wo);
    \draw[dashed,thick] (3,1.6)  -- (3,-1.4);
  \end{tikzpicture}
...
pairs for the remaining (non-local) content commons.\ednote{
    This is already outdated.
    What is the current API status?
    How is it related to FLAMS?
}
If a pair is chosen in an annotation,
